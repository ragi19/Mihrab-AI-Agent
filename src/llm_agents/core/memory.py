...
class Memory:
    ...
    def add_to_working_memory(self, item: Any, importance: float = 1.0, **metadata) -> None:
    ...
    def _consolidate_to_long_term(self, entry: MemoryEntry[Any]) -> None:
        ...
        if (self.long_term_memory_size and 
            len(self.long_term_memory) > self.long_term_memory_size):
    ...
    def summarize_working_memory(self) -> str:
        ...
        for entry in self.working_memory:
            summary.append(f"- {str(entry.content)} (importance: {entry.importance:.2f})")
        return "\n".join(summary)
    ...
    def load_state(self, file_path: str) -> None:
        ...
        self.long_term_memory = [MemoryEntry.from_dict(entry) for entry in state["long_term_memory"]]
        self.semantic_memory = {
            k: MemoryEntry.from_dict(v) for k, v in state["semantic_memory"].items()
        }
        self.episodic_memory = [MemoryEntry.from_dict(entry) for entry in state["episodic_memory"]]
    ...
    async def search(self, query: str, k: int = 5, threshold: float = 0.0) -> List[MemoryEntry]:
    ...
    async def add(self, content: Any, metadata: Dict[str, Any] = None, importance: float = 1.0) -> None:
        ...
        # Prune if needed
        if self.long_term_memory_size and len(self.long_term_memory) > self.long_term_memory_size:
