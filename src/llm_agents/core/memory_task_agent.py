...
class MemoryEnabledTaskAgent(TaskAgent):
    ...
    def __init__(
        ...
        self.logger.info(
            "Initialized memory-enabled task agent"
        )
    ...
    async def process_message(self, message: Message) -> Message:
        ...
        self._current_trace = self._trace_provider.create_trace(
            name="memory_task_agent.process_message",
    ...
    async def _process_with_tools(self, message: Message) -> Message:
        ...
        try:
            ...
            if hasattr(response, "tool_calls") and response.tool_calls:
                span.set_metadata({"has_tool_calls": True, "tool_count": len(response.tool_calls)})
                ...
                final_response = await self.model.generate_response(self.conversation_history)
    ...
    async def _augment_with_memories(self, query: str) -> None:
        ...
        """
        span = self._create_span("retrieve_memories", {
            "query": query
        })
        span.start()
        ...
        try:
            ...
            for i, msg in enumerate(self.conversation_history):
                if msg.role == MessageRole.SYSTEM and "RELEVANT MEMORIES:" in msg.content:
            ...
            self.logger.debug(f"Added memory context with {len(retrieved_memories)} memories")
        finally:
            span.end()
    ...
    def _format_memories_as_context(self, memories: List[MemoryEntry]) -> str:
        ...
        for i, entry in enumerate(memories):
            importance = f"(importance: {entry.importance:.2f})" if entry.importance else ""
            timestamp = entry.timestamp.strftime("%Y-%m-%d") if entry.timestamp else "unknown_date"
    ...
    async def _assess_importance(self, text: str) -> float:
        ...
        importance_markers = [
            ...
            "password", "account", "number", "address", "name", "date",
