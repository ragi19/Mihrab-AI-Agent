...
class Agent(ABC):
    def __init__(self, model: BaseModel, trace_provider: Optional[TraceProvider] = None):
        self.model = model
        ...
        self._post_process_hooks: List[Callable[[Message, Message], Awaitable[Message]]] = []
    ...
    async def process_message(self, message: Message) -> Message:
        ...
        try:
            ...
            with self._create_span(
                "process_message",
                {"message": processed_message.dict()}
            ) as span:
    ...
    async def add_pre_process_hook(
        self, 
        hook: Callable[[Message], Awaitable[Message]]
    ) -> None:
    ...
    async def add_post_process_hook(
        self, 
        hook: Callable[[Message, Message], Awaitable[Message]]
    ) -> None:
    ...
    def set_model(self, model: BaseModel) -> None:
        ...
        self.logger.info(f"Changing model from {self.model.model_name} to {model.model_name}")
        self.model = model
