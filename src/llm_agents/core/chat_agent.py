...
class ChatAgent(Agent):
    ...
    def __init__(
        ...
        self._history_summarizer: Optional[Callable[[List[Message]], Awaitable[str]]] = None
    ...
    async def process_message(self, message: Message) -> Message:
        ...
        try:
            ...
            with self._create_span("generate_response", {
    ...
    async def _process_message_internal(self, message: Message) -> Message:
        ...
        with self._create_span("prepare_messages", {
    ...
    async def _manage_history(self) -> None:
        ...
        if self.max_history_messages and len(self.conversation_history) > self.max_history_messages:
            await self._truncate_history_by_messages()
            
        # If we have a history summarizer, use it for long conversations
        if self._history_summarizer and len(self.conversation_history) > self.memory_window_size * 2:
    ...
    async def _truncate_history_by_tokens(self) -> None:
        ...
        for msg in reversed(other_messages):
            ...
            if total_tokens + msg_tokens <= self.max_history_tokens:
                ...
                self.logger.debug(f"Skipping message with {msg_tokens} tokens (would exceed limit)")
                break
    ...
    async def _summarize_old_history(self) -> None:
        ...
        try:
            ...
            self.conversation_history = system_messages + [summary_msg] + recent_messages
            self.logger.info(
    ...
    def set_history_summarizer(self, summarizer: Callable[[List[Message]], Awaitable[str]]) -> None:
    ...
    def set_max_history(self, max_tokens: Optional[int] = None, max_messages: Optional[int] = None) -> None:
