...
class MultiProviderModel(BaseModel):
    ...
    def __init__(
        ...
        if isinstance(selection_strategy, str):
            try:
                ...
                logger.warning(f"Invalid strategy '{selection_strategy}', using performance")
    ...
    async def generate_stream(self, messages: List[Message], **kwargs) -> AsyncIterator[Message]:
        ...
        while attempts <= self.failover_retries:
            ...
            try:
                ...
                chunk_count = 0
                async for chunk in self._current_model.generate_stream(messages, **kwargs):
                    chunk_count += 1
                    yield chunk
        ...
        while attempts <= self.failover_retries:
            ...
            try:
                ...
                if hasattr(self._current_model, 'generate_response'):
                    response = await self._current_model.generate_response(messages, tools=tools)
                else:
    ...
    async def create(cls, 
                   ...
                   trace_provider = None) -> 'MultiProviderModel':
        """
        ...
        if fallback_models:
            for model_name in fallback_models:
                try:
                    ...
                    logger.warning(f"Failed to create fallback model {model_name}: {str(e)}")
