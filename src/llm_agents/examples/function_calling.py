...
def get_current_weather(location, unit="celsius"):
    ...
    location_data = weather_data.get(location, {"temperature": 20, "condition": "Unknown"})
...
async def run_function_calling_example(provider_name, model_name, user_message):
    ...
    agent = await create_agent(
        ...
        required_capabilities={ModelCapability.FUNCTION_CALLING}  # Ensure model supports function calling
    )
    ...
    tools = [{"type": "function", "function": func_def} for func_def in available_functions.values()]
    ...
    if hasattr(response, 'tool_calls') and response.tool_calls:
        print(f"Assistant wants to call functions: {[call['name'] for call in response.tool_calls]}")
...
async def main():
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        await run_function_calling_example("anthropic", "claude-3-opus-20240229", weather_query)
    
    if os.getenv("GROQ_API_KEY"):
        # Llama models with function calling support
        await run_function_calling_example("groq", "llama-3.1-8b-instant", weather_query)
