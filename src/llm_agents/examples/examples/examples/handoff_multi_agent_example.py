...
class HandoffAgent:
    ...
    async def process(self, message: str, session_id: str = None) -> str:
        ...
        for handoff in self.handoffs:
            if self._should_handoff(message, handoff):
                ...
                return await handoff.target_agent.process(message, session_id=session_id)
    ...
    def _should_handoff(self, message: str, handoff: HandoffConfig) -> bool:
        ...
        keywords = {
            "technical": ["error", "bug", "broken", "not working", "help with", "how to", "fix"],
            "billing": ["payment", "invoice", "charge", "refund", "price", "cost", "subscription"],
            "scheduling": ["schedule", "appointment", "book", "meeting", "when", "time", "calendar"]
        }
...
class HandoffMultiAgentSystem:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"Created multi-provider model with primary: {self.model.current_provider}")
