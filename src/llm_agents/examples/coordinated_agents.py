...
async def run_coordinated_agents_example():
    ...
    research_response = await agent_group.process_with_agent("researcher", research_message)
    print(f"Research findings: {research_response.content}")
    
    # Step 2: Creative writing phase
    print("\n[CREATIVE WRITER AGENT]")
    writing_prompt = f"Based on this research, create a short story:\n{research_response.content}"
    writing_message = Message(role=MessageRole.USER, content=writing_prompt)
    story_response = await agent_group.process_with_agent("creative_writer", writing_message)
