...
async def run_vision_example(provider_name: str, model_name: str, image_path: str, prompt: str):
    """
    ...
    try:
        agent = await create_agent(
            ...
            required_capabilities={ModelCapability.VISION}  # Ensure model supports vision
        )
    except Exception as e:
    ...
    message = Message(
        ...
        content=[message_content, image_content]
    )
...
async def main():
    ...
    if not os.path.exists(image_path):
        ...
        print("Please place a sample image at this location or modify the path in the script.")
        ...
        print("\nTo use this example:")
        print(f"1. Add an image file named 'sample_image.jpg' to {os.path.dirname(image_path)}")
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        for prompt in prompts:
            await run_vision_example("openai", "gpt-4-vision-preview", image_path, prompt)
    
    if os.getenv("ANTHROPIC_API_KEY"):
        # Claude 3 models with vision support
        for prompt in prompts:
            await run_vision_example("anthropic", "claude-3-opus-20240229", image_path, prompt)
    
    if os.getenv("GROQ_API_KEY"):
        # Check if Groq has vision models available
        for prompt in prompts:
            await run_vision_example("groq", "llama-3.2-11b-vision-preview", image_path, prompt)
