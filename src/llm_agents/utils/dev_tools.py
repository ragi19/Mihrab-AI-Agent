...
class AgentProfiler:
    ...
    async def profile_agent(
            ...
            while attempt < max_retries:
                try:
                    ...
                    if hasattr(agent.model, "count_tokens"):
                        ...
                        response_tokens = await agent.model.count_tokens(response.content)
        ...
        if self.metrics["request_times"]:
            self.metrics["avg_request_time"] = (
                self.metrics["total_time"] / len(self.metrics["request_times"])
            )
        ...
        if self.metrics["token_counts"]:
            self.metrics["avg_tokens"] = (
                self.metrics["total_tokens"] / len(self.metrics["token_counts"])
            )
