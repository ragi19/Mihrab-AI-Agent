    ...
    def end(self, error: Optional[Union[Exception, SpanError]] = None) -> None:
        ...
        if error:
            if isinstance(error, Exception):
                self.error = SpanError(message=str(error), data={"type": error.__class__.__name__})
            else:
                self.error = error
        ...
        try:
            ...
            if self.append and os.path.exists(self.file_path):
                try:
                    ...
                    logger.warning(f"Could not parse existing file {self.file_path}, overwriting")
                    existing_data = []
    ...
    def create_span(
        ...
        span = Span(
            ...
            parent_id=parent_id or (
                _current_span.get().id if _current_span.get() else None
            ),
            name=name,
            data=data
        )
...
class FileTraceProvider(TraceProvider):
    ...
    def __init__(self, file_path: str, batch_size: int = 10, flush_interval: float = 5.0):
        super().__init__()
