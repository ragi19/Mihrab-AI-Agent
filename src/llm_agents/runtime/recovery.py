        ...
        async def wrapper(*args, **kwargs):
            ...
            while True:
                ...
                try:
                    ...
                    if (attempt > retry_config.max_retries or 
                        not retry_config.should_retry(e)):
                        raise
...
class MessageRetryStrategy(RecoveryStrategy):
    ...
    def __init__(self, 
                message_transformer: Callable[[Message], Message],
                max_retries: int = 3):
        ...
        try:
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    self.logger.info(f"Retry attempt {attempt} with transformed message")
    ...
    async def recover(self, error: Exception, context: Dict[str, Any]) -> bool:
        ...
        for i, strategy in enumerate(self.strategies):
            ...
            try:
                ...
                self.logger.warning(
                    f"Recovery strategy {i+1} failed with error: {e}"
                )
    ...
    async def handle_error(self, 
        ...
        for exc_type, strategy in self.strategies.items():
            if isinstance(error, exc_type):
                self.logger.info(
                    f"Using recovery strategy for {exc_type.__name__}"
                )
...
class ConversationRecovery:
    ...
    async def recover(self, error: Exception, context: Dict[str, Any]) -> Optional[Message]:
        """Attempt to recover from an error
