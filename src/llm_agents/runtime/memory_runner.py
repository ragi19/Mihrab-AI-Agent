...
class MemoryAgentRunner(AgentRunner):
    ...
    def __init__(
        ...
        self.memory_persistence_path = (
            memory_persistence_path or
            os.path.expanduser("~/.llm_agents/memories/")
        )
        ...
        logger.info(f"Initialized MemoryAgentRunner with persistence path: {self.memory_persistence_path}")
    ...
    async def run(
        ...
        self._current_trace = self.trace_provider.create_trace(
            name="memory_runner.run",
            group_id=session_id or str(id(self))
        )
